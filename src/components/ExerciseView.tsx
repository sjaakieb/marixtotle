...
function MultipleChoiceExercise({ exercise, onResult, onNext, isLast }: Props) {
  ...
  const shuffledOptions = useMemo(() => shuffle(exercise.options!), [exercise.id]);
  const correct = selected != null ? isExerciseCorrect(selected, exercise) : false;
  const isAlternative =
    submitted && correct && selected != null && normalize(selected) !== normalize(exercise.answer);
...
function TextInputExercise({ exercise, onResult, onNext, isLast }: Props) {
  ...
  const isAlternative = submitted && correct && normalize(value) !== normalize(exercise.answer);
    ...
    <div className="space-y-4">
      ...
      <form onSubmit={handleSubmit} className="space-y-3">
        ...
        {needsMacron && !submitted && (
          <MacronToolbar onInsert={handleInsert} onToggle={handleToggle} showToggle />
        )}
        {!submitted ? (
          <button
            type="submit"
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="rounded-xl bg-white p-4 ring-1 ring-stone-200">
        <div className="text-lg font-semibold text-stone-900">{exercise.prompt}</div>
...
function Feedback({
  ...
  return (
    <div
      className={[
        "rounded-xl p-4",
        correct ? "bg-emerald-50 ring-1 ring-emerald-200" : "bg-red-50 ring-1 ring-red-200",
      ].join(" ")}
    >
      <div className={correct ? "font-semibold text-emerald-800" : "font-semibold text-red-800"}>
        {correct ? "✓ Correct!" : "✗ Niet correct"}
      </div>
      {correct && isAlternative && userInput && (
        <div className="mt-1 text-sm text-emerald-700">
          Jouw antwoord <span className="font-semibold">“{userInput}”</span> is ook correct. Volledig
          antwoord: <span className="font-semibold">{answer}</span>
        </div>
      )}
      {!correct && (
      ...
      {!correct && hint && <div className="mt-1 text-sm text-stone-600">Tip: {hint}</div>}
      <button
        onClick={onNext}
        autoFocus
        className={[
          "mt-3 w-full rounded-xl px-6 py-3 font-semibold text-white",
          correct ? "bg-emerald-600 hover:bg-emerald-700" : "bg-sky-600 hover:bg-sky-700",
        ].join(" ")}
      >
