...
describe("isCorrect strict", () => {
  ...
  it("whitespace tolerant", () => {
    expect(isCorrect(" genitivus singularis ", "genitivus singularis")).toBe(true);
  });
});
...
describe("isExerciseCorrect with alternatives & implicit alias", () => {
  it("accepts exact answer", () => {
    expect(isExerciseCorrect("laetus, -a, -um", { answer: "laetus, -a, -um" })).toBe(true);
  });
  it("accepts lemma via implicit alias for adjective form", () => {
    expect(isExerciseCorrect("laetus", { answer: "laetus, -a, -um" })).toBe(true);
    expect(isExerciseCorrect("mirus", { answer: "mirus, -a, -um" })).toBe(true);
    expect(isExerciseCorrect("Laetus", { answer: "laetus, -a, -um" })).toBe(true); // case-insensitive
  });
  it("accepts explicit alternatives", () => {
    expect(
      isExerciseCorrect("laetus", { answer: "laetus, -a, -um", alternatives: ["laetus"] }),
    ).toBe(true);
    expect(
      isExerciseCorrect("laetus, -a, -um", { answer: "laetus, -a, -um", alternatives: ["laetus"] }),
    ).toBe(true);
  });
  it("accepts feminine/neuter forms via implicit alias", () => {
    expect(isExerciseCorrect("laeta", { answer: "laetus, -a, -um" })).toBe(true);
    expect(isExerciseCorrect("laetum", { answer: "laetus, -a, -um" })).toBe(true);
  ...
  it("rejects unrelated", () => {
    expect(isExerciseCorrect("laeti", { answer: "laetus, -a, -um" })).toBe(false);
    expect(isExerciseCorrect("bonus", { answer: "laetus, -a, -um" })).toBe(false);
  });
